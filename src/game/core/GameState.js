...
class GameState {
    constructor(bestScore = 0) {
        this.board = new Board_1.Board(constants_1.GAME_CONFIG.BOARD_SIZE);
        this.currentPieces = [];
        this.score = 0;
    ...
    generateNewPieces() {
        this.currentPieces = [];
        for (let i = 0; i < constants_1.GAME_CONFIG.PIECE_COUNT; i++) {
            this.currentPieces.push((0, Piece_1.generateRandomPiece)());
        }
    }
    /**
    ...
    replacePiece(index) {
        if (index >= 0 && index < this.currentPieces.length) {
            this.currentPieces[index] = (0, Piece_1.generateRandomPiece)();
        }
    }
    /**
    ...
    handleLineClearing() {
        ...
        if (fullLines.totalLines > 0) {
            ...
            const points = (0, ScoreCalculator_1.calculateScore)(emptyFields, fullLines.totalLines);
            this.score += points;
            // Update best score
